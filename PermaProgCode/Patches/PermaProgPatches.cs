...
public static class PermaProgPatches
{
    ...
    [HarmonyPrefix]
    public static void UpdateScoreInfo(NGameOverScreen __instance, string locEntryKey, string? locAmountKey,
        ref int amount, string scoreLabel, string? iconPath)
    {
    ...
    [HarmonyPrefix]
    public static void ChangeGoldScoreTextToCurrency(ref string label, string score, Texture2D? icon)
    {
        if (!label.Contains("Gold")) return;
        MF.Log.Info("Exchange end-of-run 'Gold gained' to 'Currency Gained'");
